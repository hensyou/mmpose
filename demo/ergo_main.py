...
def main():
    ...
    logger.info('Stage 0: Preprocess video.')

    adjusted_video = VideoPreprocessor.reduce_rate_and_resize(args.video_path)

    video = mmcv.VideoReader(adjusted_video)
    assert video.opened, f'Failed to load video file {adjusted_video}'

    # First stage: 2D pose detection
    logger.info('Stage 1: 2D pose detection.')
    ...
    logger.info('Stage 2: 2D-to-3D pose lifting.')
    ...
    for i, pose_det_results in enumerate(
        ...
        for idx, res in enumerate(pose_lift_results):
            # logger.info(f' looping pose_lift_results at {idx}:')
    ...
    if save_out_video:
        writer.release()
    logger.info('Generated JSON results into pose_results/')
